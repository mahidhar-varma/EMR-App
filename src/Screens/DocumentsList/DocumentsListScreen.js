  ...
  const getDocuments = async ()=>{
    try{
      ...
      console.log(configData['serverUrl']+'/documents', bodyData)
        await fetch(configData['serverUrl']+'/documents?userId='+userId+'&category='+category,
        {
            method : 'GET',
            headers: {
...
//   const recipesArray = getRecipes(item.id);
  ...
  const renderDocuments = ({ item }) => (
    <TouchableHighlight underlayColor="rgba(73,182,77,0.9)" onPress={() => onPressDocument(item)}>
  ...
  return (
    ...
    {loading &&  <View>
        <FlatList vertical showsVerticalScrollIndicator={false} numColumns={2} data={documentsList} renderItem={renderDocuments} keyExtractor={(item) => `${item.documentid}`} />
      </View>}
      </View>
